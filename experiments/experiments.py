...
class Experiment(ABC):
    ...
    def validate(self, device, epoch, save_path, x_resolution, y_resolution, z_resolution, time_resolution):
        ...
        # Check if this is a 2D model (no third dimension to slice over)
        is_2d_model = plot_config['z_axis_idx'] == -1 or self.dataset.dynamics.state_dim == 2
        
        fig = plt.figure(figsize=(5*time_resolution if is_2d_model else 5*time_resolution*len(zs), 5))
        
        for i in range(len(times)):
            # For 2D models, create just one plot per time step
            if is_2d_model:
                coords = torch.zeros(x_resolution*y_resolution, self.dataset.dynamics.state_dim + 1)
                coords[:, 0] = times[i]
                coords[:, 1 + plot_config['x_axis_idx']] = xys[:, 0]
                ...
                ax = fig.add_subplot(1, time_resolution, i+1)
                ax.set_title(f't = {times[i]:.2f}')
                s = ax.imshow(1*(values.detach().cpu().numpy().reshape(x_resolution, y_resolution).T <= 0), 
                            cmap='bwr', origin='lower', extent=(x_min, x_max, y_min, y_max))
                fig.colorbar(s)
            else:
                # Original implementation for 3D models with slicing
                for j in range(len(ys)):
                    ...
                    if plot_config['z_axis_idx'] != -1:
                        coords[:, 1 + plot_config['z_axis_idx']] = ys[j]

                    with torch.no_grad():
                        model_results = self.model({'coords': self.dataset.dynamics.coord_to_input(coords.to(device))})
                        values = self.dataset.dynamics.io_to_value(model_results['model_in'].detach(), model_results['model_out'].squeeze(dim=-1).detach())
                    
                    ax = fig.add_subplot(1, time_resolution*len(ys), (j+1) + i*len(ys))
                    if plot_config['z_axis_idx'] != -1:
                        ax.set_title(f't = {times[i]:.2f}, {plot_config["state_labels"][plot_config["z_axis_idx"]]} = {ys[j]:.2f}')
                    else:
                        ax.set_title(f't = {times[i]:.2f}')
                    s = ax.imshow(1*(values.detach().cpu().numpy().reshape(x_resolution, y_resolution).T <= 0), 
                                cmap='bwr', origin='lower', extent=(x_min, x_max, y_min, y_max))
